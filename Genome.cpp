...
#endif
{
    ...
    if ((a_SeedType != 0) && (a_NumHidden > 0))
    {
        ...
        else
        {
            ...
            int t_inp_id  = RandInt(1, a_NumInputs-1);
            int t_bias_id = a_NumInputs;
            int t_outp_id = RandInt(a_NumInputs+1, static_cast<int>(NumNeurons()));
...
void Genome::BuildHyperNEATPhenotype(NeuralNetwork& net, Substrate* subst, double a_LinkTreshold, double a_MaxWeight, double a_MinTimeConst, double a_MaxTimeConst)
{
    ...
    for(unsigned int i=net.NumInputs(); i<net.m_neurons.size(); i++)
    {
        ...
        for(unsigned int j=0; j<net.m_neurons.size(); j++)
        {
            ...
            if (t_abs_weight > a_LinkTreshold)
                // also if not connecting output<->output (optional)
                //if (!((net.m_neurons[i].m_type == OUTPUT) && (net.m_neurons[j].m_type == OUTPUT)))
                // also if not connecting hidden<->hidden (optional)
                //if (!((net.m_neurons[i].m_type == HIDDEN) && (net.m_neurons[j].m_type == HIDDEN)))
                // also if not connecting input->output (optional)
                //if (!((net.m_neurons[i].m_type == OUTPUT) && (net.m_neurons[j].m_type == INPUT)))
                // also if not connecting output->hidden (optional)
                //if (!((net.m_neurons[i].m_type == HIDDEN) && (net.m_neurons[j].m_type == OUTPUT)))
            {
                // now this weight will be scaled
                if (t_weight < 0)
                    Scale(t_weight, -1, -a_LinkTreshold, -a_MaxWeight, 0);
                else
                    Scale(t_weight, a_LinkTreshold, 1, 0, a_MaxWeight);
...
bool Genome::Mutate_AddNeuron(InnovationDatabase &a_Innovs)
{
    ...
    while(!t_link_found)
    {
        ...
        {
            t_link_num = Rounded(RandFloat());
        }
        else
        {
            //if (NumLinks() > 8)
            {
                t_link_num = RandInt(0, static_cast<int>(NumLinks()-1)); // random selection
            }
            /*else
            {
...
bool Genome::Mutate_AddLink(InnovationDatabase &a_Innovs)
{
    ...
    if (!t_MakeRecurrent)
    {
        ...
        else
        {
            ...
            do
            {
                t_n1idx = RandInt(0, static_cast<int>(NumNeurons()-1));
                t_n2idx = RandInt(t_first_noninput, static_cast<int>(NumNeurons()-1));
                t_NumTries++;
    ...
    else if (t_MakeRecurrent && !t_LoopedRecurrent)
    {
        ...
        do
        {
            t_n1idx = RandInt(t_first_noninput, static_cast<int>(NumNeurons()-1));
            t_n2idx = RandInt(t_first_noninput, static_cast<int>(NumNeurons()-1));
            t_NumTries++;
    ...
    double t_weight = RandFloatClamped();// * GlobalParameters.WeightReplacementMaxPower;
...
bool Genome::Mutate_RemoveLink()
{
    ...
    double t_randnum = RandFloat();//RandGaussClamped()/4;
    Clamp(t_randnum, 0, 1);
...
bool Genome::Mutate_RemoveSimpleNeuron(InnovationDatabase& a_Innovs)
{
    ...
    if (t_neurons_to_delete.size() == 2)
        t_choice = Rounded(RandFloat());
    else
        t_choice = RandInt(0, static_cast<int>(t_neurons_to_delete.size()-1));
...
void Genome::Mutate_LinkWeights()
{
    ...
    for(unsigned int i=0; i<NumLinks(); i++)
    {
        ...
        double t_random_choice = RandFloat();
...
void Genome::Randomize_LinkWeights(double a_Range)
{
    // For all links..
    for(unsigned int i=0; i<NumLinks(); i++)
    {
        m_LinkGenes[i].SetWeight(RandFloatClamped() * a_Range);// * GlobalParameters.WeightReplacementMaxPower);
    }
}
...
Genome Genome::Mate(Genome& a_Dad, bool a_MateAverage, bool a_InterSpecies)
{
