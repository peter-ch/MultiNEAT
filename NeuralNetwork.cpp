...
void NeuralNetwork::Adapt()
{
    ...
    for(unsigned int i=0; i<m_connections.size(); i++)
    {
        ...
        {
            double t_delta =
                (m_connections[i].m_hebb_rate * (t_max_weight - m_connections[i].m_weight)
                 * t_incoming_neuron_activation * t_outgoing_neuron_activation )
        ...
        else if (m_connections[i].m_weight < 0) // negative weight
        {
            ...
            // input is high
            double t_delta = m_connections[i].m_hebb_pre_rate * (t_max_weight - m_connections[i].m_weight)
                             * t_incoming_neuron_activation * (1.0 - t_outgoing_neuron_activation)
                             - m_connections[i].m_hebb_rate * t_max_weight * t_incoming_neuron_activation * t_outgoing_neuron_activation;

            m_connections[i].m_weight = -(m_connections[i].m_weight + t_delta);
        }

        Clamp(m_connections[i].m_weight, -GlobalParameters.MaxWeight, GlobalParameters.MaxWeight);
    }
}
...
void NeuralNetwork::RTRL_update_gradients()
{
    ...
    for(unsigned int k=m_num_inputs; k<m_neurons.size(); k++)
    {
        ...
        for(unsigned int i=m_num_inputs; i<m_neurons.size(); i++) // to
...
void NeuralNetwork::Save(FILE* a_file)
{
    ...
    for(unsigned int i=0; i<m_neurons.size(); i++)
    {
        ...
        fprintf(a_file, "neuron %d %3.18f %3.18f %3.18f %3.18f %d %3.18f\n",
                static_cast<int>(m_neurons[i].m_type),
                m_neurons[i].m_a,
                m_neurons[i].m_b,
                m_neurons[i].m_timeconst,
                m_neurons[i].m_bias,
    ...
    for(unsigned int i=0; i<m_connections.size(); i++)
    {
        ...
        fprintf(a_file, "connection %d %d %3.18f %d %3.18f %3.18f\n",
                ...
                m_connections[i].m_target_neuron_idx,
